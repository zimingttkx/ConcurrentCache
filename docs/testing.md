...
> **测试体系**：C++ 单元/集成测试 + Python E2E + GitHub Actions CI
> **入口**：`test/CMakeLists.txt`（C++ 测试目标） + `test/e2e_test/run_all_tests.py`（E2E 驱动）
> **测试统计**：12 个 C++ 可执行文件 + 9 个 Python 脚本
...
| **端到端** | Python 3.7+ + `redis-py` | 真实场景下并发 / 容错 / 混沌 |
...
> **注意**：`command_test/` 目录源码已就绪但**当前 V3 入口 `test_v3_main.cpp` 中禁用**。启用方法见 § 7。
...
**可执行文件**：`network-stress-test`（源：`network_stress_test.cpp` + `network_stress_test.h` + `network_stress_test_main.cpp`）
...
| SET 单 key | ≥ 15 万 | ≤ 2ms |
| 混合 7:2:1 | ≥ 10 万 | ≤ 5ms |
| HSET 100 fields | ≥ 5 万 | ≤ 10ms |
| LRANGE 100 元素 | ≥ 8 万 | ≤ 3ms |
| 10000 并发连接 | 服务稳定 | — |
...
| `run_all_tests.py` | 上述脚本总入口 | 取决于组合 |

**典型用法**：

```bash
python3 e2e_connection_storm.py --connections 10000 --batch-size 500
python3 e2e_high_concurrency_load.py --users 1000 --commands 100
python3 e2e_chaos_test.py   # TCP RST / 50MB payload / 畸形 RESP / 快速断开
```
...
1. 编辑 `test/test_v3_main.cpp`，取消第 11 行 `void run_all_command_tests();` 注释
2. 取消第 27、41-43、70-72 行的注释
3. 在 `test/CMakeLists.txt` 的 `concurrentcache-v3-tests` 目标中添加 `command_test/command_test.cpp`
4. 重新 `cmake --build build`
...
- [部署 § 5 Sanitizer 选项](../deployment.md)

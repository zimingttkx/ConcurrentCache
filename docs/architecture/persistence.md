...
| 阻塞主线程 | **是** | 否（fork 子进程） |
| 内存峰值 | 高（需要快照整库） | 高（fork 后父子共享页，COW 渐进） |
| 失败处理 | 返回错误给客户端 | 更新 `last_bgsave_status` |
| 实现 | `RdbPersistence::save` | `fork()` + 子进程 `save` + 父进程 `waitpid` |
| 大库影响 | 延迟 = 序列化时间 | 延迟 = fork 开销（< 10ms） |

**fork 优势**：子进程拥有父进程地址空间的快照，无需锁即可安全遍历 64 分片哈希表。

**注意**：项目当前 `save_in_background` 内部仍直接调用 `save`（同步写），并未真正使用 fork 隔离写线程；如需 fork 隔离需扩展实现。

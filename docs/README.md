...
| 文档状态 | 维护者 | 最后更新 | 适用版本 |
|---------|--------|---------|---------|
| Active | ConcurrentCache Team | 2026-06 | V3.x / V4.x |
...
│   ├── overview.md             系统分层、组件依赖、请求时序
│   ├── network.md              MainReactor / SubReactor / epoll
│   ├── storage.md              64 分片哈希 + 5 数据类型 + 过期
│   ├── memory-pool.md          ThreadCache / CentralCache / PageCache
...
| 身份 | 推荐阅读顺序 | 预估时长 |
|------|------------|---------|
| **新加入项目** | [README](../../README.md) → `architecture/overview.md` → `architecture/network.md` → `architecture/storage.md` → `api.md` | 60 min |
...
| 文档 | 内容 | 何时看 |
|------|------|-------|
| [`overview.md`](architecture/overview.md) | 6 层组件图、模块依赖、请求时序、关键不变量 | **第一份必读** |
...
| 文档 | 内容 | 何时看 |
|------|------|-------|
| [`api.md`](api.md) | 全部命令语法、参数、RESP 响应、错误码、客户端示例 | 写客户端代码、查命令格式 |

### 3.3 测试

| 文档 | 内容 | 何时看 |
|------|------|-------|
| [`testing.md`](testing.md) | 12 个 C++ 测试套件、Python E2E 脚本、Sanitizer 用法、CI 流程 | 加新功能写测试、跑回归 |

### 3.4 部署与运维

| 文档 | 内容 | 何时看 |
|------|------|-------|
| [`deployment.md`](deployment.md) | 编译选项、Docker 部署、CI 流程、配置项、故障排查 | 上线、调配置、查问题 |

...
| **字符串命令 (V2)** | | | |
| PING | `PING` | PONG | ✅ |
| EXPIRE | `EXPIRE key seconds` | Integer (1成功/0失败) | ✅ |
| TTL | `TTL key` | Integer (秒数/-1永久/-2不存在) | ✅ |
| PTTL | `PTTL key` | Integer (毫秒数) | ✅ |
| PERSIST | `PERSIST key` | Integer (1成功/0失败) | ✅ |
| SETEX | `SETEX key seconds value` | OK | ✅ |
| **LIST 命令 (V3)** | | | |
| LPUSH | `LPUSH key value [value ...]` | Integer (列表长度) | ✅ |
| RPUSH | `RPUSH key value [value ...]` | Integer (列表长度) | ✅ |
| LPOP | `LPOP key` | Bulk String 或 Nil | ✅ |
| RPOP | `RPOP key` | Bulk String 或 Nil | ✅ |
| LLEN | `LLEN key` | Integer (长度) | ✅ |
| LRANGE | `LRANGE key start stop` | Array | ✅ |
| **HASH 命令 (V3)** | | | |
| HSET | `HSET key field value` | Integer (1新增/0更新) | ✅ |
| HGET | `HGET key field` | Bulk String 或 Nil | ✅ |
| HDEL | `HDEL key field [field ...]` | Integer (删除数量) | ✅ |
| HLEN | `HLEN key` | Integer (字段数量) | ✅ |
| HGETALL | `HGETALL key` | Array (field-value pairs) | ✅ |
| **SET 命令 (V3)** | | | |
| SADD | `SADD key member [member ...]` | Integer (新增数量) | ✅ |
| SPOP | `SPOP key [count]` | Bulk String 或 Array | ✅ |
| SCARD | `SCARD key` | Integer (成员数量) | ✅ |
| SISMEMBER | `SISMEMBER key member` | Integer (1是/0否) | ✅ |
| SMEMBERS | `SMEMBERS key` | Array (所有成员) | ✅ |
| **ZSET 命令 (V3)** | | | |
| ZADD | `ZADD key score member [score member ...]` | Integer (新增/更新数量) | ✅ |
| ZSCORE | `ZSCORE key member` | Bulk String (分数) 或 Nil | ✅ |
| ZCARD | `ZCARD key` | Integer (成员数量) | ✅ |
| ZRANGE | `ZRANGE key start stop [WITHSCORES]` | Array | ✅ |
| **RDB 命令 (V3.1)** | | | |
| SAVE | `SAVE` | OK | ✅ |
| BGSAVE | `BGSAVE` | OK 或 Background saving started | ✅ |
| LASTSAVE | `LASTSAVE` | Integer (时间戳) | ✅ |

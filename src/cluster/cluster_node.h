...
public:
    ...
    void incrementFailureCount() { failure_count_.fetch_add(1, std::memory_order_relaxed); }                        // 增加失败计数
    void resetFailureCount() { failure_count_.store(0, std::memory_order_relaxed); }                        // 重置失败计数
    [[nodiscard]] int getFailureCount() const { return failure_count_.load(std::memory_order_relaxed); }    // 获取失败计数
    void setFirstPfailTime(int64_t time) { first_pfail_time_.store(time, std::memory_order_relaxed); }    // 设置首次疑似下线时间
    [[nodiscard]] int64_t getFirstPfailTime() const { return first_pfail_time_.load(std::memory_order_relaxed); }  // 获取首次疑似下线时间
    void setFailTime(int64_t time) { fail_time_.store(time, std::memory_order_relaxed); }                 // 设置客观下线时间
    [[nodiscard]] int64_t getFailTime() const { return fail_time_.load(std::memory_order_relaxed); }       // 获取客观下线时间

    // 故障转移相关
    void setFailoverState(FailoverState state) { failover_state_.store(static_cast<int>(state), std::memory_order_relaxed); }  // 设置故障转移状态
    [[nodiscard]] FailoverState getFailoverState() const { return static_cast<FailoverState>(failover_state_.load(std::memory_order_relaxed)); }  // 获取故障转移状态
    void setFailoverEpoch(int64_t epoch) { failover_epoch_.store(epoch, std::memory_order_relaxed); }  // 设置故障转移轮次
    [[nodiscard]] int64_t getFailoverEpoch() const { return failover_epoch_.load(std::memory_order_relaxed); }  // 获取故障转移轮次
    void addVote(const std::string& node_name, int64_t epoch, int64_t offset);  // 添加投票
    [[nodiscard]] int getVoteCount() const { std::lock_guard<std::mutex> lock(votes_mutex_); return static_cast<int>(votes_.size()); }  // 获取票数
    [[nodiscard]] bool hasVoted() const { std::lock_guard<std::mutex> lock(votes_mutex_); return !votes_.empty(); }  // 是否已投票
    void clearVotes() { std::lock_guard<std::mutex> lock(votes_mutex_); votes_.clear(); }  // 清除投票
    [[nodiscard]] int64_t getMaxVotedOffset() const;  // 获取最大投票偏移量
    void setReplicaPriority(int priority) { replica_priority_.store(priority, std::memory_order_relaxed); }  // 设置从节点优先级
    [[nodiscard]] int getReplicaPriority() const { return replica_priority_.load(std::memory_order_relaxed); }  // 获取从节点优先级
    void setFailoverStartTime(int64_t time) { failover_start_time_.store(time, std::memory_order_relaxed); }  // 设置故障转移开始时间
    [[nodiscard]] int64_t getFailoverStartTime() const { return failover_start_time_.load(std::memory_order_relaxed); }  // 获取故障转移开始时间
...
private:
    ...
    // 故障检测相关
    std::atomic<int> failure_count_{0};     // PING 超时次数
    std::atomic<int64_t> first_pfail_time_{0};  // 首次疑似下线时间
    std::atomic<int64_t> fail_time_{0};         // 客观下线确认时间

    // 故障转移相关
    std::atomic<int> failover_state_{static_cast<int>(FailoverState::kNoFailover)};  // 故障转移状态
    std::atomic<int64_t> failover_epoch_{0};           // 故障转移轮次
    std::vector<VoteInfo> votes_;                      // 收集的投票（由 votes_mutex_ 保护）
    mutable std::mutex votes_mutex_;                    // 保护 votes_
    std::atomic<int> replica_priority_{100};           // 从节点优先级
    std::atomic<int64_t> failover_start_time_{0};      // 故障转移开始时间（毫秒）
};
